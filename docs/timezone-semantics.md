...
Visible meeting times, email template time variables, and local timestamps currently use a 24-hour clock (`00:00`–`23:59`) in every UI language. The email editor previews `scheduleIcsUrl` with the saved task's public URL only after the task has an ID and enables `serveScheduleIcs`.
