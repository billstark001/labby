import { buildEntityPageQueries, listGraphPage } from '@labby/db';
...
export class LabbyStore {
  ...
  private async listEntityPage<T>(table: 'persons' | 'person_tags' | 'keywords', query: ListQuery): Promise<PaginatedResult<T>> {
    ...
    const { pageSql, countSql, offset, limit } = buildEntityPageQueries(table, query, collation);
    const [rows, totals] = await Promise.all([
      this.queryRows(sql.raw(pageSql)),
      this.queryRows(sql.raw(countSql)),
    ]);
    return {
